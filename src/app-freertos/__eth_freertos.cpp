...
#include "lwipserver/network/eth_freertos.h"
...
SemaphoreHandle_t RxPktSemaphore = nullptr; /* Semaphore to signal incoming packets */
SemaphoreHandle_t TxPktSemaphore = nullptr; /* Semaphore to signal transmit packet complete */
...
static bool inHandlerMode(void) {
    return __get_IPSR() != 0;
}

static bool semRelease(SemaphoreHandle_t sem) {
    if (inHandlerMode()) {
    ...
    } else if (xSemaphoreGive(sem) != pdTRUE) {
        return false;
    }
    return true;
}

static bool semWait(SemaphoreHandle_t sem, uint32_t millisec) {
    if (inHandlerMode()) {
        ...
        portEND_SWITCHING_ISR(taskWoken);
    } else if (xSemaphoreTake(sem, pdMS_TO_TICKS(millisec)) != pdTRUE) {
        return false;
    }
    return true;
}
...
static void low_level_init(struct netif *netif) {
    ...
    auto status =
        xTaskCreate(ethernetif_input, "EthIf", INTERFACE_THREAD_STACK_SIZE, netif, PRIORITY_REAL_TIME, nullptr);
    if (status == pdFAIL) {
        while (true);
    }
...
void ethernetif_input(void *argument) {
    ...
    while (true) {
        if (semWait(RxPktSemaphore, TIME_WAITING_FOR_INPUT) == true) {
            while (true) {
